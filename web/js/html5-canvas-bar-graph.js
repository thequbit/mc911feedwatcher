...
function BarGraph(ctx) {
  ...
  // Draw method updates the canvas with the current display
	  ...
	  for (i = 0; i < arr.length; i += 1) {
		...
		if (that.xAxisLabelArr[i]) {					
		  ...
		  try{
				...
				if( that.rotateAxisText == true )
				{
					drawRotatedText(that.xAxisLabelArr[i],
					  (i * that.width / numOfBars + (that.width / numOfBars) / 2)-45,
					  that.height - 10);
	...
	var drawRotatedText = function(text, x, y)
	{
		...
		var tx = x + (metric.width/2)+6;
		var ty = y - metric.width;
